...
public class ExcelRenderer extends Renderer{

	public File file;
	private XSSFWorkbook wb;

	XSSFCellStyle styleRight;
	XSSFCellStyle styleRight1;
	XSSFCellStyle styleRight2;
	XSSFCellStyle styleRight3;
	XSSFCellStyle styleRight4;
	XSSFCellStyle styleRight5;
	XSSFCellStyle styleRight6;
	XSSFCellStyle styleRight7;
	XSSFCellStyle styleRight8;
	XSSFCellStyle styleRight9;

	XSSFCellStyle styleRH;
	XSSFCellStyle styleCD;
	XSSFCellStyle styleRD;
	XSSFCellStyle styleRD_first;
	XSSFCellStyle styleRD_first_end;
	XSSFCellStyle styleRD_merge;
	XSSFCellStyle styleRD_merge_end;
	...
	private final Color rhColor = new Color(188, 204, 224);
	private final Color cdColor = new Color(240, 235, 215);
	private final Color rdColor = new Color(226, 236, 248);
	...
	public ExcelRenderer(ParamInfo paramInfo, Map<List<String>,  Measure> resultMap, RowAxis rowAxis, ColumnAxis columnAxis, boolean levelExpr, CmmtInfoManager cmmtInfoManager, StatHtmlDAO statHtmlDAO, HttpServletRequest request){
	...
	public void write(){
		...
		if(paramInfo.getAnalTextTblNm() != null){
			fileName += "_" + dateString + "_" + analTitle + "(" + paramInfo.getAnalTextTblNm().replaceAll("<", "").replaceAll(">", "").replaceAll(" ", "_").replaceAll("/", "_") + ").xlsx";
		}else{
	...
	private void makeExcel(){
		XSSFSheet sheetData = wb.createSheet(sheetName);	// 데이터 시트생성
		XSSFRow hssfRow     = null;		// row
		XSSFCell hssfCell   = null;		// cell
	...
	private void addMetaSheet(){
		XSSFSheet sheetMeta = wb.createSheet(sheetMetaName);	// 메타 시트생성
		XSSFRow hssfRow     = null;		// row
		XSSFCell hssfCell   = null;		// cell
		short cellIdx   = 0;		// 셀 index
		short rowIdx    = 0;		// row index
		...
		XSSFCellStyle styleMeta = wb.createCellStyle();	// default cell style
		styleMeta.setAlignment(XSSFCellStyle.ALIGN_LEFT);
	...
	private void setCellValue(XSSFCell cell, XSSFRow row, XSSFCellStyle style, short cellIdx, String dataType, String cellText, int cellType)
	{
		...
		XSSFRichTextString str = new XSSFRichTextString(replace_cellText); // str에 [{}] 제거한 cellText값을 셋팅
		...
		if(dataType != null){
			if(dataType.equals("Num")){
				...
				if(smblYn.equals("Y")){ //2019.03.12 통계부호를 볼때만 윗첨자 작업된 셀값을 보여준다
					...
					cell.setCellValue(new XSSFRichTextString(cellText)); // 통계부호를 보지 않을때는 이미 셀값에 들어가 있는 부호를 제거하고 보여준다.
				}
			}
			
		}else{
			// 통계부호관련해서 따로 컨트롤하는 부분이 안보이므로 [{}] 만 떼서 보여준다...2019.03 이전버전에 [{}] 없을때처럼 넣어주기위해서 replace한 값을 넣어줌
			cell.setCellValue(new XSSFRichTextString(replace_cellText));   
		}
	}

	private void setCellValue1(XSSFCell cell, XSSFRow row, XSSFCellStyle style, short cellIdx, String dataType, String cellText)
	{
		cell = row.createCell((int) cellIdx);
		...
		if(dataType != null){
			if(dataType.equals("Num")){
				...
				cell.setCellValue(new XSSFRichTextString(cellText));
			}
		}
		else{
			cell.setCellValue(new XSSFRichTextString(cellText));
	...
	private void appendRHStyle(){
		...
		styleRH.setFillPattern(XSSFCellStyle.SOLID_FOREGROUND);
		styleRH.setVerticalAlignment((short)1);
		styleRH.setFillForegroundColor(new XSSFColor(rhColor));
		styleRH.setBorderBottom(XSSFCellStyle.BORDER_THIN);
		styleRH.setBorderLeft(XSSFCellStyle.BORDER_THIN);
		styleRH.setBorderRight(XSSFCellStyle.BORDER_THIN);
		styleRH.setBorderTop(XSSFCellStyle.BORDER_THIN);
	}

	private void appendCDStyle(){
		styleCD = wb.createCellStyle();
		styleCD.setFillPattern(XSSFCellStyle.SOLID_FOREGROUND);
		styleCD.setFillForegroundColor(new XSSFColor(cdColor));
		styleCD.setBorderBottom(XSSFCellStyle.BORDER_THIN);
		styleCD.setBorderLeft(XSSFCellStyle.BORDER_THIN);
		styleCD.setBorderRight(XSSFCellStyle.BORDER_THIN);
		styleCD.setBorderTop(XSSFCellStyle.BORDER_THIN);
	}

	private void appendRDStyle(){
		styleRD = wb.createCellStyle();
		styleRD.setFillPattern(XSSFCellStyle.SOLID_FOREGROUND);
		styleRD.setFillForegroundColor(new XSSFColor(rdColor));
		styleRD.setBorderBottom(XSSFCellStyle.BORDER_THIN);
		styleRD.setBorderLeft(XSSFCellStyle.BORDER_THIN);
		styleRD.setBorderRight(XSSFCellStyle.BORDER_THIN);
		styleRD.setBorderTop(XSSFCellStyle.BORDER_THIN);
	}

	private void appendRD_first_Style(){
		styleRD_first = wb.createCellStyle();
		styleRD_first.setFillPattern(XSSFCellStyle.SOLID_FOREGROUND);
		styleRD_first.setFillForegroundColor(new XSSFColor(rdColor));
		styleRD_first.setBorderLeft(XSSFCellStyle.BORDER_THIN);
		styleRD_first.setBorderRight(XSSFCellStyle.BORDER_THIN);
		styleRD_first.setBorderTop(XSSFCellStyle.BORDER_THIN);
	}

	private void appendRD_first_end_Style(){
		styleRD_first_end = wb.createCellStyle();
		styleRD_first_end.setFillPattern(XSSFCellStyle.SOLID_FOREGROUND);
		styleRD_first_end.setFillForegroundColor(new XSSFColor(rdColor));
		styleRD_first_end.setBorderBottom(XSSFCellStyle.BORDER_THIN);
		styleRD_first_end.setBorderLeft(XSSFCellStyle.BORDER_THIN);
		styleRD_first_end.setBorderRight(XSSFCellStyle.BORDER_THIN);
		styleRD_first_end.setBorderTop(XSSFCellStyle.BORDER_THIN);
	}

	private void appendRD_merge_Style(){
		styleRD_merge = wb.createCellStyle();
		styleRD_merge.setFillPattern(XSSFCellStyle.SOLID_FOREGROUND);
		styleRD_merge.setFillForegroundColor(new XSSFColor(rdColor));
		styleRD_merge.setBorderLeft(XSSFCellStyle.BORDER_THIN);
		styleRD_merge.setBorderRight(XSSFCellStyle.BORDER_THIN);
	}

	private void appendRD_merge_end_Style(){
		styleRD_merge_end = wb.createCellStyle();
		styleRD_merge_end.setFillPattern(XSSFCellStyle.SOLID_FOREGROUND);
		styleRD_merge_end.setFillForegroundColor(new XSSFColor(rdColor));
		styleRD_merge_end.setBorderBottom(XSSFCellStyle.BORDER_THIN);
		styleRD_merge_end.setBorderLeft(XSSFCellStyle.BORDER_THIN);
		styleRD_merge_end.setBorderRight(XSSFCellStyle.BORDER_THIN);
	}
	...
	private void appendValueStyle(){
		...
		styleRight1.setAlignment(XSSFCellStyle.ALIGN_RIGHT);
		styleRight1.setBorderBottom(XSSFCellStyle.BORDER_THIN);
		styleRight1.setBorderLeft(XSSFCellStyle.BORDER_THIN);
		styleRight1.setBorderRight(XSSFCellStyle.BORDER_THIN);
		styleRight1.setBorderTop(XSSFCellStyle.BORDER_THIN);
		styleRight1.setDataFormat(wb.createDataFormat().getFormat("#,##0.0"));

		styleRight2 = wb.createCellStyle();;	// 오른쪽정렬  cell style
		styleRight2.setAlignment(XSSFCellStyle.ALIGN_RIGHT);
		styleRight2.setBorderBottom(XSSFCellStyle.BORDER_THIN);
		styleRight2.setBorderLeft(XSSFCellStyle.BORDER_THIN);
		styleRight2.setBorderRight(XSSFCellStyle.BORDER_THIN);
		styleRight2.setBorderTop(XSSFCellStyle.BORDER_THIN);
		styleRight2.setDataFormat(wb.createDataFormat().getFormat("#,##0.00"));

		styleRight3 = wb.createCellStyle();;	// 오른쪽정렬  cell style
		styleRight3.setAlignment(XSSFCellStyle.ALIGN_RIGHT);
		styleRight3.setBorderBottom(XSSFCellStyle.BORDER_THIN);
		styleRight3.setBorderLeft(XSSFCellStyle.BORDER_THIN);
		styleRight3.setBorderRight(XSSFCellStyle.BORDER_THIN);
		styleRight3.setBorderTop(XSSFCellStyle.BORDER_THIN);
		styleRight3.setDataFormat(wb.createDataFormat().getFormat("#,##0.000"));

		styleRight4 = wb.createCellStyle();;	// 오른쪽정렬  cell style
		styleRight4.setAlignment(XSSFCellStyle.ALIGN_RIGHT);
		styleRight4.setBorderBottom(XSSFCellStyle.BORDER_THIN);
		styleRight4.setBorderLeft(XSSFCellStyle.BORDER_THIN);
		styleRight4.setBorderRight(XSSFCellStyle.BORDER_THIN);
		styleRight4.setBorderTop(XSSFCellStyle.BORDER_THIN);
		styleRight4.setDataFormat(wb.createDataFormat().getFormat("#,##0.0000"));

		styleRight5 = wb.createCellStyle();;	// 오른쪽정렬  cell style
		styleRight5.setAlignment(XSSFCellStyle.ALIGN_RIGHT);
		styleRight5.setBorderBottom(XSSFCellStyle.BORDER_THIN);
		styleRight5.setBorderLeft(XSSFCellStyle.BORDER_THIN);
		styleRight5.setBorderRight(XSSFCellStyle.BORDER_THIN);
		styleRight5.setBorderTop(XSSFCellStyle.BORDER_THIN);
		styleRight5.setDataFormat(wb.createDataFormat().getFormat("#,##0.00000"));
		
		styleRight6 = wb.createCellStyle();;	// 오른쪽정렬  cell style
		styleRight6.setAlignment(XSSFCellStyle.ALIGN_RIGHT);
		styleRight6.setBorderBottom(XSSFCellStyle.BORDER_THIN);
		styleRight6.setBorderLeft(XSSFCellStyle.BORDER_THIN);
		styleRight6.setBorderRight(XSSFCellStyle.BORDER_THIN);
		styleRight6.setBorderTop(XSSFCellStyle.BORDER_THIN);
		styleRight6.setDataFormat(wb.createDataFormat().getFormat("#,##0.000000"));
		
		styleRight7 = wb.createCellStyle();;	// 오른쪽정렬  cell style
		styleRight7.setAlignment(XSSFCellStyle.ALIGN_RIGHT);
		styleRight7.setBorderBottom(XSSFCellStyle.BORDER_THIN);
		styleRight7.setBorderLeft(XSSFCellStyle.BORDER_THIN);
		styleRight7.setBorderRight(XSSFCellStyle.BORDER_THIN);
		styleRight7.setBorderTop(XSSFCellStyle.BORDER_THIN);
		styleRight7.setDataFormat(wb.createDataFormat().getFormat("#,##0.0000000"));
		
		styleRight8 = wb.createCellStyle();;	// 오른쪽정렬  cell style
		styleRight8.setAlignment(XSSFCellStyle.ALIGN_RIGHT);
		styleRight8.setBorderBottom(XSSFCellStyle.BORDER_THIN);
		styleRight8.setBorderLeft(XSSFCellStyle.BORDER_THIN);
		styleRight8.setBorderRight(XSSFCellStyle.BORDER_THIN);
		styleRight8.setBorderTop(XSSFCellStyle.BORDER_THIN);
		styleRight8.setDataFormat(wb.createDataFormat().getFormat("#,##0.00000000"));
		
		styleRight9 = wb.createCellStyle();;	// 오른쪽정렬  cell style
		styleRight9.setAlignment(XSSFCellStyle.ALIGN_RIGHT);
		styleRight9.setBorderBottom(XSSFCellStyle.BORDER_THIN);
		styleRight9.setBorderLeft(XSSFCellStyle.BORDER_THIN);
		styleRight9.setBorderRight(XSSFCellStyle.BORDER_THIN);
		styleRight9.setBorderTop(XSSFCellStyle.BORDER_THIN);
