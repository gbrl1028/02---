 ...
 *   2014.09.26	        김정아		jsonObject null체크 (null이면 true, 아니면 false)
 *
 * </pre>
 */
...
public class StringUtil {
	...
	/**
	 * 문자열( ${101|DT_1F01001|C:B} )에서 기관코드, 통계표ID, 분류코드를 가져온다.
	 *
	 * @param str
	 *            원본 문자열
	 * @return 기관코드, 통계표ID, 분류코드 배열
	 */
	public static String[] getClsInfoArr(String str) {
	...
	public static String null2str2(String str) {
		if (str == "null" || str.length() == 0){
			return "";
		}else{
			return str.trim();
	...
	public static String null2str(String str, String converted) {
    	if (str == null || str.equals("")){
			return converted;
		}else{
			return str.trim();
...
/*				case 10:
	...
	public static String toFileName(String fileName, int len){
		String rsltName = "";
		if(fileName.indexOf("\\") > 0){
			fileName = fileName.substring(0, len);
			len 	 = fileName.lastIndexOf("\\");
			rsltName = fileName.substring(len+1);
		}else{
			rsltName = fileName.substring(0, len);
		}
		return rsltName;
	}
	 ...
	 * @return 바뀌어진 값을 넘겨준다.
	 */
	public static String removeSpecialCharforFile(String str) {
		String value = null;
		...
		value = value.replaceAll("/", "_"); // '/' 치환
		value = value.replaceAll(",", "_"); // ',' 치환
		value = value.replaceAll("\\(", "_"); // '(' 치환
		value = value.replaceAll("\\)", "_"); // ')' 치환
		value = value.replaceAll(" ", "_"); // 가운데 공백 치환
	...
	/**
	 * 변조(CSRF)된 String 원래대로 변경
	 *
	 * @param inputString 입력문자열
	 * @return String Script 문제를 대체한 결과 문자열
	 * @throws Exception
	 */
	public static String restoreTag(String data) {
        if (data == null || data.trim().equals("")) {
            return "";
        }
        String ret = data;
	 ...
	 * @throws Exception
	 */
    public static String getCSRF(String value) {
		...
		value = value.replaceAll("(?i)<.*?JAVA-SCRIPT:.*?>.*?</.*?>", "");
	...
	public static String bbsIdReplace(String str)
	{
	...
	/**
	 * jsonObject null체크 (null이면 true, 아니면 false)
	 * @param str
	 * @return
	 */
	public static boolean jsonObjectNullCheck(String str){
	...
	/**
	 * @Method 설명	: special character(특수문자)를  변환 합니다. 
	 * @param		: @param s
	 * @param		: @return
	 * @return 		: String
	 */
	public static String replace_str(String str) throws Exception { 
		String str_out = "";
		str_out = str.replaceAll("'", "`");
